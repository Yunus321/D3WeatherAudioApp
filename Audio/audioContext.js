...
const frequencies = [440,330,297,264,396]
...
function conversionAndSumOfFrequencies(_currentGlyphValues) { 
    ...
    .range([0, 2]) // max amplitude of each frequency
    ...
    for (var i = 0; i < _currentGlyphValues.length; i++) {
        ...
        for (var j = 0; j < ctx.sampleRate; j++) {
            arr[j] = calculateSineWave(j,frequencies[i],ctx) *  calcAmplitude(_currentGlyphValues[i]) //_currentGlyphValues[j] giv the amplitude of each data 
        }
